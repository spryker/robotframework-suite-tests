...
test_ui_b2b:
	robot -v env:$(ENV_UI_B2B) -v glue_env:${glue_env} -v yves_env:${yves_env} -v yves_at_env:${yves_at_env} -v zed_env:${zed_env} -v mp_env:${mp_env} --exclude skip-due-to-issueORskip-due-to-refactoring -d results tests/ui/e2e/e2e_b2b.robot
...
test_ui_suite:
	robot -v env:$(ENV_UI_SUITE) -v glue_env:${glue_env} -v yves_env:${yves_env} -v yves_at_env:${yves_at_env} -v zed_env:${zed_env} -v mp_env:${mp_env} --exclude skip-due-to-issueORskip-due-to-refactoring -d results tests/ui/e2e/e2e_suite.robot
