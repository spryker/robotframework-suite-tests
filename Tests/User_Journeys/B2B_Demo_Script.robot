...
Quick_Order
    ...
    #  Тест падает, так как первая карта -это линка. Я поменяла xpath на //*[contains(@class,'icon--cart')]/ancestor::li//div[contains(@class,'js-user-navigation__sub-nav-cart')]//div[@class='mini-cart-detail']//a/*[text()='${shoppingCartName}']
    Yves: 'Shopping Cart' page is displayed
    # Yves: shopping cart contains the following products:    401627    520561    101509    419871    419869    425073    425084
    # Изменила количество продуктов (401627,1\n520561,3\n101509,21\n419871,1\n419869,11\n425073,1\n425084,2), так как трешолд не проходили. Степ зависимый от трешолда и демо данных
    Yves: go to 'Quick Order' page through the header
    ...
    # Заменила адрес на  Ms Sonia Wagner, Oderberger Str. 57, 10115 Berlin (зависимый от тест данных) тут лучше создавать для теста новый BU адрес и его использовать
    Yves: select the following shipping method on the checkout and go next:    Express
    Yves: select the following payment method on the checkout and go next:    Invoice
    # Новый метод для нажатия чекбокса на чекауте Yves: Accept the Terms and Conditions:    true
    Yves: 'submit the order' on the summary page
