...
Create_warehouse_user_assigment:
    [Documentation]    This keyword creates new warehouse user assigment in the DB table `spy_warehouse_user_assignment`.
        ...    *Example:*
        ...
        ...    ``Create_warehouse_user_assigment:   ${warehouse[0].warehouse_uuid}    ${warehouse_user[0].user_uuid}    false``
        ...
...
Get_warehouse_user_assigment_id:
    [Documentation]    This keyword retrive  warehouse user assigment  ${id_warehouse_user_assigment} in the DB table `spy_warehouse_user_assignment` by uuuid and user_uuid.
        ...    *Example:*
        ...
        ...    ``Get_warehouse_user_assigment_id:   ${warehouse[0].warehouse_uuid}    ${warehouse_user[0].user_uuid}`
        ...
    [Arguments]    ${warehouse_uuid}    ${user_uuid}
    Connect to Spryker DB
    ${id_warehouse_user_assigment}=   Query    SELECT id_warehouse_user_assignment FROM spy_warehouse_user_assignment WHERE uuid = '${warehouse[0].warehouse_uuid}' AND user_uuid='${warehouse_user[0].user_uuid}';
    Disconnect From Database
...
Remove_warehouse_user_assigment:
    [Documentation]    This keyword deletes the entry from the DB table `spy_warehouse_user_assignment`.
        ...    *Example:*
        ...
        ...    ``Remove_warehouse_user_assigment:    ${warehouse[0].warehouse_uuid}    ${warehouse_user[0].user_uuid}``
        ...
    [Arguments]    ${warehouse[0].warehouse_uuid}    ${warehouse_user[0].user_uuid}
        ${id_warehouse_user_assigment}=    Get_warehouse_user_assigment_id:   ${warehouse[0].warehouse_uuid}    ${warehouse_user[0].user_uuid}
    Connect to Spryker DB
