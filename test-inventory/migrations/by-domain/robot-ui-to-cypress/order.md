...
| [x] | Refunds | ×5 | Checks that refund can be created for one item and the whole order. _(yves)_ | `cypress/e2e/backoffice/order-management/order-refund.cy.ts::given a delivered order of three items when they are refunded one by one then each refund is recorded and the grand total reaches zero` | L | `local 2026-08-25 · 1 passing · 34s` |
