...
def render_progress(config: dict, matrices: dict[str, list[dict]]) -> str:
    lines = [
        ...
        "",
        "Status is observed from **this branch's** working tree, so a branch that has not done the "
        "work reports 0 ported. That is accurate, not stale.",
        "",
        "| Matrix | Domains | Scenarios | left to port | in scope | ported | authored "
        "| target skipped | dropped | review | blocked | done % |",
        "|---|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|",
    ]
    for name, rows in matrices.items():
        ...
        lines += [
            f"| {name} | {domains} | {len(rows)} | {migrate - ported} | {migrate} | {ported} "
            f"| {authored} | {skipped} "
            f"| {dropped} | {review} | {blocked} | {percent} |"
        ]

    for name, rows in matrices.items():
        lines += ["", f"## {name} by domain", "",
                  "| Domain | Scenarios | left to port | in scope | ported | authored "
                  "| target skipped | dropped | review | blocked | batches |",
                  "|---|--:|--:|--:|--:|--:|--:|--:|--:|--:|---|"]
        ...
        for domain in sorted(by_domain):
            ...
            lines.append(
                f"| [{domain}](by-domain/{name}/{domain}.md) | {len(subset)} "
                f"| {migrate - ported} | {migrate} | {ported} "
                f"| {authored} | {skipped} "
