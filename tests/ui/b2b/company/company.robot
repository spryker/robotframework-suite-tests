...
Create_new_company_with_linked_entities_and_customer_in_backoffice
    ...
    Zed: create new Zed user with the following data:    agent+company_user_bo${random}@acme.com    ${default_secure_password}    Agent    Assist    Root group    This user is an agent in Storefront    en_US
    Yves: go to the 'Home' page
    Yves: logout on Yves as a customer
    Yves: go to URL:    agent/login
    Yves: login on Yves with provided credentials:    agent+company_user_bo${random}@acme.com    ${default_secure_password}    agent_assist=${True}
...
Create_new_company_user_with_linked_entities_in_storefront
    ...
    Zed: login on Zed with provided credentials:    ${zed_admin_email}
    Zed: create new Zed user with the following data:    agent+company_user_yves${random}@acme.com    ${default_secure_password}    Agent    Assist    Root group    This user is an agent in Storefront    en_US
    Yves: go to the 'Home' page
    Yves: logout on Yves as a customer
    Yves: go to URL:    agent/login
    Yves: login on Yves with provided credentials:    agent+company_user_yves${random}@acme.com    ${default_secure_password}    agent_assist=${True}
