...
Term and conditions on the summary page
    [Documentation]  Checks submit button is disabled without check t&c checkbox and t&c page translation in EN and DE 
    Yves: login on Yves with provided credentials:    ${yves_company_user_buyer_email}
    Yves: create new 'Shopping Cart' with name:    mycart+${random}
    Yves: go to PDP of the product with sku:    M64933
    Yves: add product to the shopping cart
    Yves: go to the shopping cart through the header with name:    mycart+${random}
    Yves: click on the 'Checkout' button in the shopping cart
    Yves: select the following existing address on the checkout as 'shipping' address and go next:    ${yves_company_user_buyer_address}
    Yves: select the following shipping method on the checkout and go next:    Express
    Yves: select the following payment method on the checkout and go next:    Invoice    
    Yves: check states of 'Submit your order' button:    false
    Yves: accept the terms and conditions:    true
    Yves: check states of 'Submit your order' button:    true
    Yves: click on term and conditions   
    ${previous_page}=    Switch Page    NEW
    Yves: change current locale language:    DE
    Yves: verify current locale language:    DE
