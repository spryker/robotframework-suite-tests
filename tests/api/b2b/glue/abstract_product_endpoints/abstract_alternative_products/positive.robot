...
Product_has_abstract_alternative
    When I send a GET request:    /concrete-products/${concrete.alternative_products.product_1.sku}/abstract-alternative-products
    ...
    And Response body parameter should be:    [data][0][attributes][sku]    ${abstract.alternative_products.product_2.sku}
    And Response body has correct self link

Product_has_abstract_alternative_with_includes
    [Setup]    Run Keywords    I get access token for the customer:    ${yves_user.email}
    ...    AND    I set Headers:    Content-Type=${default_header_content_type}    Authorization=${token}  
    When I send a GET request:    /concrete-products/${concrete.alternative_products.product_1.sku}/abstract-alternative-products?include=abstract-product-image-sets,abstract-product-availabilities,abstract-product-prices,category-nodes
    Then Response status code should be:    200
    And Response reason should be:    OK
    And Response header parameter should be:    Content-Type    ${default_header_content_type}
    And Response should contain the array of a certain size:    [data]    3
...
Product_has_no_abstract_alternative
    When I send a GET request:    /concrete-products/${bundle_product.product_2.concrete_sku}/abstract-alternative-products
