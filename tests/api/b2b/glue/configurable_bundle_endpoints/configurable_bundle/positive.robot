...
Get_configurable_bundle_templates_by_configurable_bundle.template_id
    When I send a GET request:    /configurable-bundle-templates/${configurable_bundle.template_id}?include=configurable-bundle-template-slots,configurable-bundle-template-image-sets
    Then Response status code should be:    200
    And Response reason should be:    OK
    And Response header parameter should be:    Content-Type    ${default_header_content_type}
...
Add_configured_bundle_item_to_the_cart_with_included_items
  [Setup]    Run Keywords    I get access token for the customer:    ${yves_user.email}
    ...
    When I send a POST request:    /carts/${cart_id}/configured-bundles?include=items    {"data": {"type": "configured-bundles","attributes": {"quantity": ${configurable_bundle.quantity},"templateUuid": "${configurable_bundle.template_id}","items": [{"sku": "${configurable_bundle.first_slot_item_sku}","quantity": 2,"slotUuid": "${configurable_bundle.first_slot_uuid}"}]}}}
    ...
    And Response body parameter should be:    [included][0][attributes][sku]    ${configurable_bundle.first_slot_item_sku}
