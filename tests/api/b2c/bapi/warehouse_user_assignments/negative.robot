...
Create_warehouse_user_assigment_as_warehouse_user_for_other_user
    [Setup]    Run Keywords    I get access token by user credentials:    ${warehouse_user_assigment.user_name}
    ...
    When I send a POST request:    /warehouse-user-assignments    {"data": {"type": "warehouse-user-assignments", "attributes":{"userUuid": "${warehouse_user_assigment.user_uuid_2}","warehouse" :{"uuid": "${warehouse.warehouse_uuid}"},"isActive":"false"}}}
...
Create_warehouse_user_assigment_with_invalid_body
    ...
    ...    AND    I set Headers:    Content-Type=${default_header_content_type}    Authorization=Bearer ${token}
    When I send a POST request:    /warehouse-user-assignments    {"data": {"type": "warehouse-user-assignments", "attributes":{"userUuid": "test","warehouse" :{"uuid": "${warehouse.warehouse_uuid}"},"isActive":"false"}}}
    Then Response status code should be:    400
...
Create_warehouse_user_assigment_with_incorrect_type
    ...
    ...    AND    I set Headers:    Content-Type=${default_header_content_type}    Authorization=Bearer ${token}
    ...    AND    Make user a warehouse user/ not a warehouse user:   ${warehouse_user_assigment.user_uuid}    1
    When I send a POST request:    /warehouse-user-assignments    {"data": {"type": "invalid", "attributes":{"userUuid": "${warehouse_user_assigment.user_uuid}","warehouse" :{"uuid": "${warehouse.warehouse_uuid}"},"isActive":"false"}}}
    Then Response status code should be:    400
    And Make user a warehouse user/ not a warehouse user:   ${warehouse_user_assigment.user_uuid}    0
...
Get_warehouse_user_assigments_by_UUID_without_token
    ...
    [Tags]    skip-due-to-issue
    And Make user a warehouse user/ not a warehouse user:   ${warehouse_user_assigment.user_uuid}    1
    And Create_warehouse_user_assigment:    ${warehouse.warehouse_uuid}    ${warehouse.fk_warehouse_spryker}    ${warehouse_user_assigment.user_uuid}    false
    Then Get_warehouse_user_assigment_id:   ${warehouse.warehouse_uuid}    ${warehouse_user_assigment.user_uuid}
    Then I send a GET request:    /warehouse-user-assignments/${id_warehouse_user_assigment}
    Then Response status code should be:    403
    And Remove_warehous_user_assigment:    ${warehouse.warehouse_uuid}    ${warehouse_user_assigment.user_uuid}
    And Make user a warehouse user/ not a warehouse user:   ${warehouse_user_assigment.user_uuid}    0
...
Get_user_assigments_list_with_invalid_token
    ...
    And Response should return error message:    Invalid access token.
    And Remove_warehous_user_assigment:    ${warehouse.warehouse_uuid}    ${warehouse_user_assigment.user_uuid}
    And user a warehouse user/ not a warehouse user:   ${warehouse_user_assigment.user_uuid}    0
