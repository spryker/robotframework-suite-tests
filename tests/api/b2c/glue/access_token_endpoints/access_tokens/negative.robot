...
Get_acess_token_with_invalid_email
    When I send a POST request:    /access-tokens    {"data":{"type":"access-tokens","attributes":{"username":"fake@spryker.com","password":"${yves_user_password}"}}}
    Then Response status code should be:    401
    And Response reason should be:    Unauthorized
    And Response should return error code:    003
    And Response should return error message:    Failed to authenticate user.
...
Get_acess_token_with_invalid_type
    When I send a POST request:    /access-tokens    {"data":{"type":"access","attributes":{"username":"${yves_second_user_email}","password":"${yves_second_user_password}"}}}
    Then Response status code should be:    400
    And Response reason should be:    Bad Request
    And Response should return error message:    Invalid type.

Get_acess_token_with_empty_type
    When I send a POST request:    /access-tokens    {"data":{"type":"","attributes":{"username":"${yves_second_user_email}","password":"${yves_second_user_password}"}}}
    Then Response status code should be:    400
    And Response reason should be:    Bad Request
    And Response should return error message:    Invalid type.
