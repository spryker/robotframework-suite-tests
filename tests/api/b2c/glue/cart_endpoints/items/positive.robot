...
Add_one_item_to_cart
    ...
    When I send a POST request:    /carts/${cart_id}/items    {"data": {"type": "items","attributes": {"sku": "${add_cart_item_sku}", "quantity": "1"}}}
...
Add_two_items_to_cart_with_included_items_concrete_products_and_abstract_products
   [Setup]    Run Keywords    I get access token for the customer:    ${yves_user_email}
          ...
          ...  AND    Find or create customer cart
          ...  AND    Cleanup all items in the cart:    ${cart_id}
    When I send a POST request:    /carts/${cart_id}/items?include=items,concrete-products,abstract-products    {"data": {"type": "items","attributes": {"sku": "${add_cart_item_sku}", "quantity": "1"}}}
    Then Response status code should be:    201
    And Response reason should be:    Created
    And Response header parameter should be:    Content-Type    ${default_header_content_type}
    And Response body parameter should be:    [data][id]    ${cart_id}
    ...
    And Response body parameter should be:    [included][2][id]    ${add_cart_item_sku}
    And Response body parameter should be:    [included][2][attributes][sku]    ${add_cart_item_sku}
    And Response body parameter should be:    [included][2][attributes][quantity]    1
    And Response body parameter should be:    [included][2][attributes][groupKey]    ${add_cart_item_sku}
    And Response body parameter should be:    [included][2][attributes][abstractSku]    ${add_cart_item_abstract_sku}
...
Get_a_cart_with_included_items_and_concrete_products
    ...
    And Response body parameter should be:    [included][1][id]    ${add_cart_item_sku}
    And Response body parameter should be:    [included][1][attributes][sku]    ${add_cart_item_sku}
    And Response body parameter should be:    [included][1][attributes][quantity]    2
    And Response body parameter should be:    [included][1][attributes][groupKey]    ${add_cart_item_sku}
    And Response body parameter should be:    [included][1][attributes][abstractSku]    ${add_cart_item_abstract_sku}
...
Add_five_items_to_cart_with_included_cart_rules_and_promotional_items
    [Setup]    Run Keywords    I get access token for the customer:    ${yves_user_email}
                  ...
                  ...  AND    Find or create customer cart
                  ...  AND    Cleanup all items in the cart:    ${cart_id}
    When I send a POST request:    /carts/${cart_id}/items?include=cart-rules,promotional-items   {"data": {"type": "items","attributes": {"sku": "${add_cart_item_sku}","quantity": 5}}}
    Then Response status code should be:    201
    And Response reason should be:    Created
    And Response header parameter should be:    Content-Type    ${default_header_content_type}
    And Response body parameter should be:    [data][id]    ${cart_id}
...
Add_product_with_options_to_cart
     [Setup]    Run Keywords    I get access token for the customer:    ${yves_user_email}
          ...
          ...  AND    Find or create customer cart
          ...  AND    Cleanup all items in the cart:    ${cart_id}
    When I send a POST request:    /carts/${cart_id}/items?include=items    {"data":{"type": "items","attributes":{"sku": "${add_cart_item_sku}","quantity": 1,"productOptions": [{ "sku": "${product_option_1}"},{ "sku": "${product_option_2}"}] }}}
    Then Response status code should be:    201
    And Response reason should be:    Created
    And Response header parameter should be:    Content-Type    ${default_header_content_type}
    And Response body parameter should be:    [data][id]    ${cart_id}
    ...
    And Response include should contain certain entity type:    items
    And Response include element has self link:   items
    And Response body parameter should contain:    [included][0][id]    ${add_cart_item_sku}
    And Response body parameter should be:    [included][0][attributes][sku]    ${add_cart_item_sku}
    And Response body parameter should contain:    [included][0][attributes][groupKey]    ${add_cart_item_sku}
...
Change_item_qty_in_cart
    ...
    When I send a PATCH request:    /carts/${cart_id}/items/${add_cart_item_sku}?include=items  {"data":{"type": "items","attributes":{"quantity": 2}}}
    Then Response status code should be:    200
    And Response reason should be:    OK
    And Response header parameter should be:    Content-Type    ${default_header_content_type}
    And Response body parameter should be:    [data][id]    ${cart_id}
...
Delete_item_form_cart
     [Setup]    Run Keywords    I get access token for the customer:    ${yves_user_email}
          ...
          ...  AND    Find or create customer cart
          ...  AND    Cleanup all items in the cart:    ${cart_id}
          ...  AND    I send a POST request:    /carts/${cart_id}/items?include=items    {"data": {"type": "items","attributes": {"sku": "${add_cart_item_sku}", "quantity": "1"}}}
