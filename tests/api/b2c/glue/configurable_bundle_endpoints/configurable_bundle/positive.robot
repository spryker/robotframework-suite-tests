...
Update_configured_bundle_quantity_in_the cart_to_the_cart
  [Setup]    Run Keywords    I get access token for the customer:    ${yves_second_user.email}
    ...
    When I send a PATCH request:    /carts/${cart_id}/configured-bundles/${bundle_group_key}?include=items    {"data": {"type": "configured-bundles","attributes": {"quantity": "8","templateUuid": "${configurable_bundle_template_1_uuid}","items": [{"sku": "${configurable_bundle_first_slot_item_sku}","quantity": "8","slotUuid": "${configurable_bundle_slot_1_uuid}"}]}}}
