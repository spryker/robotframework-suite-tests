...
Adding_item_with_abstract_sku
    ...
    When I send a POST request:   /wishlists/${wishlist_reference_id}/wishlist-items    {"data": {"type": "wishlist-items","attributes": {"sku": "${abstract_available_product_with_stock}"}}}
...
Adding_item_with_invalid_wishilist_id
    [Setup]    Run Keywords    I get access token for the customer:    ${yves_user_email}
    ...    AND    I set Headers:    Authorization=${token}
    When I send a POST request:   /wishlists/Mywishlist/wishlist-items    {"data": {"type": "wishlist-items","attributes": {"sku": "${concrete_available_product_with_stock}"}}}
    Then Response status code should be:    422
    And Response reason should be:    Unprocessable Content  
    And Response should return error code:    206 
    And Response should return error message:        "Cant add an item."
...
Adding_item_without_wishilist_id
    [Setup]    Run Keywords    I get access token for the customer:    ${yves_user_email}
    ...    AND    I set Headers:    Authorization=${token}
    When I send a POST request:   /wishlists//wishlist-items    {"data": {"type": "wishlist-items","attributes": {"sku": "${concrete_available_product_with_stock}"}}}
...
Deleting_item_which_is_not_exist_in_wishlist
    ...
    ...    AND    Response reason should be:    Created
    ...    AND    Save value to a variable:    [data][id]    wishlist_reference_id
    When I send a DELETE request:    /wishlists/${wishlist_reference_id}/wishlist-items/${concrete_available_product_with_stock}
    Then Response status code should be:    404
    And Response reason should be:    Not Found
    And Response should return error code:    208
    And Response should return error message:    No item with provided sku in wishlist.
...
Delete_wishlist_item_from_already_deleted_wishlist
    ...
    ...    AND    Response reason should be:    Created
    ...    AND    Save value to a variable:    [data][id]    wishlist_reference_id
    When I send a POST request:    /wishlists/${wishlist_reference_id}/wishlist-items    {"data": {"type": "wishlist-items","attributes": {"sku": "${concrete_available_product_with_stock}"}}}
...
Deleting_item_without_wishilist_id
    [Setup]    Run Keywords    I get access token for the customer:    ${yves_user_email}
    ...    AND    I set Headers:    Authorization=${token}
    When I send a DELETE request:   /wishlists//wishlist-items/${concrete_available_product_with_stock}
    Then Response status code should be:    400
    And Response reason should be:    Bad Request
    And Response should return error message:    Resource id is not specified.
...
Deleting_concrete_product_by_abstract_product_sku
    ...
    ...    AND    Response reason should be:    Created
    ...    AND    Save value to a variable:    [data][id]    wishlist_id
    ...    AND    I send a POST request:    /wishlists/${wishlist_id}/wishlist-items    {"data": {"type": "wishlist-items","attributes": {"sku": "${concrete_available_product_sku}"}}}
    ...    AND    Response status code should be:    201 
    ...    AND    Response reason should be:    Created
    When I send a DELETE request:    /wishlists/${wishlist_id}/wishlist-items/${abstract_available_product_with_stock}
    Then Response status code should be:    404
    And Response reason should be:    Not Found
    And Response should return error code:    208
    And Response should return error message:        No item with provided sku in wishlist.
