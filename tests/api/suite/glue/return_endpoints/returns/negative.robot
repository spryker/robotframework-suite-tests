...
Create_a_return_with_order_is_not_returnable_for_merchant
    ...
    ...  AND    I send a POST request:    /carts/${cartId}/items?include=items    {"data": {"type": "items","attributes": {"sku": "${merchants.sony_experts.concrete_product_with_offer_sku}","quantity": 10, "merchantReference" : "${merchants.sony_experts.merchant_id}", "productOfferReference" : "${merchants.sony_experts.merchant_offer_id}"}}}
