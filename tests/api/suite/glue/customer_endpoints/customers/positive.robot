...
Create_customer
    ...
    ...    /customers/
    ...    {"data":{"type":"customers","attributes":{"firstName":"${yves_third_user.first_name}","lastName":"${yves_third_user.last_name}","gender":"${gender.male}","salutation":"${yves_third_user.salutation}","email":"${email.name}+${random}${email.domain}","password":"${yves_user.password}","confirmPassword":"${yves_user.password}","acceptedTerms":True}}}
    ...
    ...    select registration_key from spy_customer where customer_reference = '${user_id}'
    ...    confirmation_key
...
New_customer_can_login_after_confirmation
    ...
    I send a POST request:
    ...    /access-tokens
    ...    {"data":{"type":"access-tokens","attributes":{"username":"${userEmail}","password":"${yves_user.password}"}}}
    And Response status code should be:    201
