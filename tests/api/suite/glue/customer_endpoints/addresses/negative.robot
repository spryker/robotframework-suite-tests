...
Patch_customer_address_without_id
    ...
    ...    /customers/${yves_user.reference}/addresses
    ...    {"data": {"type": "addresses","attributes": {"salutation": "${yves_second_user.salutation}","address1": "${changed.address1}","address2": "${changed.address2}","address3": "${changed.address3}","phone": "${changed.phone}"}}}
    Then Response status code should be:    400
    And Response reason should be:    Bad Request
...
Patch_customer_address_with_no_reference
    ...
    ...    {"data": {"salutation": "${yves_second_user.salutation}","type": "addresses","attributes": {"address1": "${changed.address1}","address2": "${changed.address2}","address3": "${changed.address3}","phone": "${changed.phone}"}}}
    Then Response status code should be:    400
    And Response reason should be:    Bad Request
    And Response should return error message:    Resource id is not specified.
...
Patch_customer_address_with_empty_required_fields
    ...
    When I send a PATCH request:
    ...    /customers/${yves_user.reference}/addresses/${address_uid}
    ...    {"data": {"salutation": "${yves_second_user.salutation}","type": "addresses","attributes": {"salutation": None,"firstName": None,"lastName": None, "address1": None,"address2": None,"zipCode": None,"city": None,"iso2Code": None}}}
    Then Response status code should be:    ${422}
    And Response reason should be:    Unprocessable Content
    And Each array element of array in response should contain property with value:    [errors]    code    901
    And Each array element of array in response should contain property with value:    [errors]    status    ${422}
