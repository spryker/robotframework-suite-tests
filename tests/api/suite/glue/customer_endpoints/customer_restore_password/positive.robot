...
Restore_password_with_all_required_fields_and_valid_data
    [Setup]    Run Keywords    I send a POST request:    /customers/    {"data":{"type":"customers","attributes":{"firstName":"${yves_eighth_user.first_name}","lastName":"${yves_eighth_user.last_name}","gender":"${gender.female}","salutation":"${yves_eighth_user.salutation}","email":"${email.name}${random}${email.domain}","password":"${yves_eighth_user.password_new}","confirmPassword":"${yves_eighth_user.password_new}","acceptedTerms":True}}}
