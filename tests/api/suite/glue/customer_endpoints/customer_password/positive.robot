...
Update_customer_password_with_all_required_fields_and_valid_data
    [Setup]    Run Keywords    I send a POST request:    /access-tokens    {"data":{"type":"access-tokens","attributes":{"username":"${yves_eighth_user.email}","password":"${yves_eighth_user.password_new}"}}}
    ...
    ...    AND    I send a PATCH request:    /customer-password/${yves_eighth_user.reference}   {"data":{"type":"customer-password","attributes":{"password":"${yves_eighth_user.password_new_additional}","newPassword":"${yves_eighth_user.password_new}","confirmPassword":"${yves_eighth_user.password_new}"}}}
