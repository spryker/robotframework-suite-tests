...
Add_an_item_to_the_guest_cart_with_items_include
    ...
    ...    AND    Cleanup All Items In The Guest Cart:    ${guest_cart_id}
    When I send a POST request:
    ...    /guest-carts/${guestCartId}/guest-cart-items?include=items
...
Add_an_item_to_the_guest_cart_with_concrete_products_and_abstract_products_includes
    ...
    ...    AND    Cleanup All Items In The Guest Cart:    ${guest_cart_id}
    When I send a POST request:
    ...    /guest-carts/${guestCartId}/guest-cart-items?include=concrete-products,abstract-products
    ...    {"data":{"type":"guest-cart-items","attributes":{"sku":"${concrete_product_with_concrete_product_alternative.sku}","quantity":"1"}}}
    Then Response status code should be:    201
    And Response reason should be:    Created
    And Response body parameter should not be EMPTY:    [data][links][self]
...
Add_an_item_to_the_guest_cart_with_cart_rules_includes
    ...
    ...    AND    Cleanup All Items In The Guest Cart:    ${guest_cart_id}
    When I send a POST request:
    ...    /guest-carts/${guestCartId}/guest-cart-items?include=cart-rules
    ...    {"data":{"type":"guest-cart-items","attributes":{"sku":"${concrete_product_with_concrete_product_alternative.sku}","quantity": 10}}}
    Then Response status code should be:    201
    And Response reason should be:    Created
    And Response header parameter should be:    Content-Type    ${default_header_content_type}
    And Response body parameter should be:    [data][id]    ${guestCartId}
...
Add_an_item_to_the_guest_cart_with_bundle_items_include
    ...
    ...    AND    Cleanup All Items In The Guest Cart:    ${guest_cart_id}
    When I send a POST request:
    ...    /guest-carts/${guestCartId}/guest-cart-items?include=bundle-items,bundled-items
    ...    {"data":{"type":"guest-cart-items","attributes":{"sku":"${bundled_product.concrete.product_1_sku}","quantity":"1"}}}
    Then Response status code should be:    201
    And Response reason should be:    Created
    And Response body parameter should not be EMPTY:    [data][links][self]
    And Response body parameter should be:
...
Update_an_item_quantity_at_the_guest_cart_with_items_include
    ...
    ...    /guest-carts/${guestCartId}/guest-cart-items/${item_id_guest}?include=items
