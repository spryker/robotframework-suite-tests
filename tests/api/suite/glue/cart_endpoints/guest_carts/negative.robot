...
Update_guest_cart_with_invalid_type
    ...
    When I send a PATCH request:
    ...    /guest-carts/${guest_cart_id}
    ...    {"data": {"type": "guest-car","attributes": {"priceMode": "${mode.gross}","currency": "${currency.eur.code}","store": "${store.de}"}}}
    Then Response status code should be:    400
    And Response reason should be:    Bad Request
...
Update_guest_cart_without_type
    ...
    When I send a PATCH request:
    ...    /guest-carts/${guest_cart_id}
    ...    {"data": {"attributes": {"priceMode": "${mode.gross}","currency": "${currency.eur.code}","store": "${store.de}"}}}
    Then Response status code should be:    400
    And Response reason should be:    Bad Request
...
Update_guest_cart_update_price_mode_with_items_in_the_cart
    ...
    When I send a PATCH request:
    ...    /guest-carts/${guest_cart_id}
    ...    {"data": {"type": "guest-carts","attributes": {"priceMode": "${mode.net}"}}}
    ...
    ...    [errors][0][detail]
    ...    Can’t switch price mode when there are items in the cart.
