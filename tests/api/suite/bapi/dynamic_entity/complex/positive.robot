...
Create_product_abstract_collection_with_child_stock_products:
    ...
    And I set Headers:    Content-Type=application/json    Authorization=Bearer ${token}
    And I send a POST request:    /dynamic-entity/robot-tests-products    {"data": [{"fk_product_abstract": 1,"is_active": 1,"attributes": "FOO","sku": "FOO2","robotTestsProductStockProducts": [{"fk_stock": 1,"is_never_out_of_stock": 1,"quantity": 0}]}]}
    Then Response status code should be:    201
    And Save value to a variable:    [data][0][id_product]    id_product
    And Save value to a variable:    [data][0][sku]    concrete_sku
    And Save value to a variable:    [data][0][robotTestsProductStockProducts][0][id_stock_product]    id_stock_product
    Trigger p&s
