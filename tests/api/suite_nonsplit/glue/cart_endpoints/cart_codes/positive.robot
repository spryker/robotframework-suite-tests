...
Delete_gift_card_from_cart
    ...
    ...  AND    Response status code should be:    201
    ...  AND    Save value to a variable:    [data][id]    cart_id
    ...  AND    I send a POST request:    /carts/${cart_id}/items?include=items    {"data": {"type": "items","attributes": {"sku": "${concrete_available_product.with_label}","quantity": 1}}}
...
Add_gift_card_code_to_the_guest_cart
    ...
    ...    AND    Save value to a variable:    [data][id]    guestcart_id
        ...    AND     Create giftcode in Database:    ${random}    ${gift_card.amount}
    When I send a POST request:    /guest-carts/${guestcart_id}/cart-codes?include=vouchers,gift-cards     {"data": {"type": "cart-codes","attributes": {"code": "${random}"}}}
...
Delete_gift_card_code_from_the_guest_cart
    ...
    ...    AND    Save value to a variable:    [data][id]    guestcart_id
    ...    AND     Create giftcode in Database:    ${random}    ${gift_card.amount}
     ...    AND    I send a POST request:    /guest-carts/${guestcart_id}/cart-codes?include=vouchers,gift-cards     {"data": {"type": "cart-codes","attributes": {"code": "${random}"}}}
    ...    AND    Response status code should be:    201
    ...    AND    Save value to a variable:    [included][0][id]    cart_code_id
