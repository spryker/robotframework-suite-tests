...
Create_a_return_with_Invalid_access_token
    ...
    When I send a POST request:     /returns     {"data":{"type":"returns","attributes":{"store":"DE","returnItems":[{"salesOrderItemUuid":"${random}","reason":"${return_reason_damaged}"}]}}}
...
Create_return_for_order_item_that_cannot_be_returned
    ...
    ...  AND    I send a POST request:    /carts    {"data":{"type":"carts","attributes":{"priceMode":"${currency.mode.gross}","currency":"${currency.eur.code}","store":"${store.de}","name": "${test_cart_name}-${random}"}}}
...
Create_a_return_with_order_is_not_returnable_for_merchant
    ...
    ...  AND    I send a POST request:    /carts/${cartId}/items    {"data": {"type": "items","attributes": {"sku": "${merchants.sony_experts.concrete_product_with_offer_sku}","quantity": 10, "merchantReference" : "${merchants.sony_experts.merchant_id}", "productOfferReference" : "${merchants.sony_experts.merchant_offer_id}"}}}
...
Create_return_without_returnItems_uuid
     [Setup]    Run Keywords    I get access token for the customer:    ${yves_second_user.email}
    ...    AND    I set Headers:    Authorization=${token}
    When I send a POST request:     /returns     {"data":{"type":"returns","attributes":{"store":"DE","returnItems":[{"salesOrderItemUuid":"","reason":"${return_reason_damaged}"}]}}}
    Then Response status code should be:     422
    And Response reason should be:     Unprocessable Content
    And Response should return error message:    "Return cant be created."    
    And Response should return error code:    3601
