...
Create_order_with_net_mode_&_chf_currency_&_express_shipment_method
    ...
    ...  AND    Save value to a variable:    [data][id]    cart_id
    ...  AND    I send a POST request:    /carts/${cartId}/items    {"data": {"type": "items","attributes": {"sku": "${abstract_product.product_availability.concrete_available_with_stock_and_never_out_of_stock}","quantity": 15, "merchantReference" : "${merchants.merchant_spryker_id}"}}}
...
Create_order_with_weight_product_&_product_options
    ...
    ...  AND    I send a POST request:    /carts/${cartId}/items    {"data": {"type": "items","attributes": {"sku": "${abstract_product.product_availability.concrete_available_with_stock_and_never_out_of_stock}","quantity": 1, "merchantReference" : "${merchants.merchant_spryker_id}"}}}
    ...  AND    I send a POST request:    /carts/${cartId}/items    {"data": {"type": "items","attributes": {"sku": "${abstract_product.product_with_options.concrete_sku}","quantity": 1, "merchantReference" : "${merchants.merchant_spryker_id}","productOptions": [{"sku": "${product_options.option_1}"}]}}}
    ...  AND    I send a POST request:    /carts/${cartId}/items    {"data": {"type": "items","attributes": {"sku": "${concrete_product_random_weight.sku}","quantity": 1,"merchantReference" : "${merchants.merchant_spryker_id}","salesUnit": {"id": "${sales_unit_id}","amount": 10}}}}
