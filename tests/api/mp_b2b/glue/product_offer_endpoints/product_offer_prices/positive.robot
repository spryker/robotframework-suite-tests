...
Get_concrete_product_without_offers_prices
    When I send a GET request:    /concrete-products/${bundle_product_concrete_sku}/product-offers?include=product-offer-prices
...
Get_all_concrete_product_offer_info_with_product_offer_prices_and_product_offer_availabilities_and_merchants_included
    When I send a GET request:    /concrete-products/${concrete_product_with_volume_prices}/product-offers?include=product-offer-prices,merchants
     Then Response status code should be:    200
    And Response reason should be:    OK
...
Get_product_offer_price_with_gross_eur_volume_prices
    When I send a GET request:    /product-offers/${offer_with_volume_price}/product-offer-prices?priceMode=${gross_mode}
    Then Response status code should be:    200
    And Response reason should be:    OK
    And Response header parameter should be:    Content-Type    ${default_header_content_type}
...
Get_product_offer_price_with_net_eur_volume_prices
    When I send a GET request:    /product-offers/${offer_with_volume_price}/product-offer-prices?priceMode=${net_mode}
    Then Response status code should be:    200
    And Response reason should be:    OK
    And Response header parameter should be:    Content-Type    ${default_header_content_type}
    And Response body parameter should be:    [data][0][id]    ${offer_with_volume_price}
